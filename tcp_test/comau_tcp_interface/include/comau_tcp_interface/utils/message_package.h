 ...
 */
        ...
        public:
            using _type_variants = boost::variant<bool, char, uint8_t, uint32_t, int32_t, float, double, vector6f_t, vector6d_t,
                                                  vector6i_t, vectorImage_t, trajectoryf_t, trajectoryd_t>;
            /**
   ...
   */
            template <typename T>
            bool setData(const std::string &name, T &value)
            {
                if (data_.find(name) != data_.end()) {
                    data_[name] = value;
                } else {
   ...
   */
            ...
            {
                template <typename T>
                void operator()(T &d, MessageParser &mp) const
                {
                    mp.parse(d);
                }
            };
