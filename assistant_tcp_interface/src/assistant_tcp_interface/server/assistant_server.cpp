...
bool AssistantServer::initialize(const AssistantTcpServerInterfaceParameters &params) {
  is_connected_ = false;
  // Assign parameters
  params_ptr_.reset(new AssistantTcpServerInterfaceParameters(params));
  ...
  /*
    ...
    try {
      receiving_thread_.reset(new std::thread(&AssistantServer::callback, this, std::move(future_obj_for_exit_)));
  ...
  */
  tcp_server_interface_ptr_.reset(new AssistantTcpServerInterface(*params_ptr_));
  return true;
}

void AssistantServer::close() {
  is_connected_ = false;
...
bool AssistantServer::isConnected() {
  return is_connected_;
}

bool AssistantServer::getLastMessage(MessagePackage &msg) {
...
void AssistantServer::receive(std::chrono::milliseconds timeout) {

  std::vector<uint8_t> recv_raw_data(last_recv_msg_->getCapacity());
  size_t read_len = 0;

  tcp_server_interface_ptr_->read(recv_raw_data, sizeof(recv_raw_data), read_len);
  if (read_len == 0) {
...
bool AssistantServer::send(MessagePackage &msg) {
  ...
  tcp_server_interface_ptr_->write(send_buffer, serielized_buffer_size, written_size);
...
void AssistantServer::callback(std::future<void> exit_signal) {
...
bool AssistantServer::sendMessage() {
...
PLUGINLIB_EXPORT_CLASS(assistant_tcp_interface::AssistantServer, assistant_tcp_interface::AssistantServerBase)
